...
export async function getLevelDistinctCountForAdmin() {
  const user = await auth();
  const userId = user?.user.id;
  const intermidarecoursecount = await db.courseUser.count({
    where: {
...
export async function getTheoriginofsubscriptionuserforAdmin() {
  const subscriptionuser = await db.user.findMany({
    //distinct user how purchased the course
  ...
  const subscriptionusers = [
    ...
    {
      userCount: subscriptionuser.length- (userfromafricacount+userfromasiacount+userfromeuropecount+userfromamericacount),
      region: "Other",
    },
  ];
